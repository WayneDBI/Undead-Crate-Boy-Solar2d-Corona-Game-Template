...
local function nextLevelPlayPrepare()
	level = nextLevel						-- LEVEL is assigned the value from the NEXTLEVEL value from the previous game
...
function scene:create( event )
	...
	----------------------------------------------------------------------------------------------------
	local image = display.newImage( imagePath.."gameLevelCompleteScreen.png",480,320 )
	image.x = _w/2
	image.y = _h/2
	...
	local nextLevelButton = widget.newButton{
		left 	= 288-45,
		top 	= 258-45,
		defaultFile = imagePath.."button_NextLevel.png",
		overFile 	= imagePath.."button_NextLevelOn.png",
		onRelease = nextLevelPlayPrepare,
		}			
	--nextLevelButton:setReferencePoint(display.CenterReferencePoint)
	nextLevelButton.anchorX = 0.5		-- Graphics 2.0 Anchoring method
	nextLevelButton.anchorY = 0.5		-- Graphics 2.0 Anchoring method
	screenGroup:insert( nextLevelButton )
end
